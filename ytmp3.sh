...
if [ -z "$2" ]
  then
    ...
    $docker_command yt-dlp -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best" --write-subs --write-auto-sub --sub-lang "en.vtt" --restrict-filenames --yes-playlist -o "m_files/%(title)s.%(ext)s" "$1"
fi
